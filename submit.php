...
<?php 
	...
	function readSpecifcTimeTrades(&$fileName){
		if (fopen("alerts/".$fileName, "r") === false) {	
			...
			if (feof($file_handle)) {
				throw new Exception('</br> File is empty.');
			} else {
				...
				while (!feof($file_handle) ) { // While loop, to open all of the files 	
					...
					echo "<tr> <td width='15%;'> ".$trades[0] . " </td>";
					echo "<td width='12%;'> ".$trades[1] ." </td>";
					echo "<td width='12%;'> ".$trades[2] ." </td>" ; // Printing tde results
					echo "<td width='5%;'> ".$trades[3] ." </td>" ; // Printing tde results		<br>
					echo "<td width='7%;'> ".$trades[4] ." </td>" ; // Printing tde results		<br>
					echo "<td width='6%;'> ".$trades[5] ." </td>" ; // Printing tde results		<br>
					echo "<td width='5%;'> ".$trades[6] ." </td>" ; // Printing tde results		<br>
					echo "<td width='12%;'> ".$trades[7] ." </td>" ; // Printing tde results		<br>
					echo "<td width='5%;'> ".$trades[8] ." </td>" ; // Printing tde results		<br>
					echo "<td width='5%;'> ".$trades[9] ." </td>" ; // Printing tde results		<br>
					echo "</tr>";
				}	
				echo "</table>";
			}
	...
	function readSpecficComms(&$fileName) {
		if (fopen("alerts/".$fileName, "r") === false) {	
			...
			if (feof($file_handle)) {
				...
				while (!feof($file_handle) ) { // While loop, to open all of the files 	
					$comms = fgetcsv($file_handle, 1024);  // Array of results
					echo "<tr> <td width='20%;'> ".$comms[0]. "  </td>";
					echo "<td width='20%;'> ".$comms[1] ." </td>";
					echo "<td width='60%;'> ".$comms[2] ." </td>" ; // Printing the results
					echo "</tr>";
				} 
				echo "</table>";
			}
	...
	function readLog() {
		if (fopen("logfile.csv", "r") === false) {	
			...
			while (!feof($file_handle) ) { // While loop, to open all of the files 	
				...
				echo "<tr> <td width='20%;'> ".$logs[0]. "  </td>";
				echo "<td width='20%;'> ".$logs[1] ." </td>";
				echo "<td width='20%;'> ".$logs[2] ." </td>" ; // Printing the results
				echo "<td width='20%;'> ".$logs[3] ." </td>" ; // Printing the results
				echo "<td width='20%;'> ".$logs[4] ." </td>" ; // Printing the results
				echo "</tr>";
			} 
			echo "</table>";
			fclose($file_handle); // close
